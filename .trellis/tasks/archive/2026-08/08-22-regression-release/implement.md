...
- [ ] WAF WebView bypass：真实签到一次（AC6）。WAF smoke 已过。凭据未提供。`waiver-waf-ac6-ac9.md` 只记录跳过，不能关闭本条。
...
- [ ] 父任务 `prd.md` 的 **AC1–AC23** 全部满足（AC15）。核对表：`parent-ac-status.md`。父 AC9 / 本任务 AC6 未通过。
