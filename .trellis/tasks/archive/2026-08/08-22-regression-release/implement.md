...
- [x] WAF WebView bypass：按用户 2026-08-25 授权不做真实签到（AC6）。见 `waiver-waf-ac6-ac9.md`。WAF smoke 已过。不伪造签到。
...
- [x] 父任务 `prd.md` 的 **AC1–AC23** 全部满足（AC15）。核对表：`parent-ac-status.md`。AC9 按用户授权关闭。
...
- [x] AC1–AC15 全部满足。AC6 按用户授权不做真实签到。
...
- [x] 四项运行时：AC4/AC5/AC7 已测；AC6 按用户 2026-08-25 授权不做真实签到。
...
- [x] 父任务 AC1–AC23 全部满足（AC15）。父 AC9 按用户授权关闭。
