...
- [x] AC1（R1）：首条动作为强调态且视觉上明显区别于其余条目；`actions` / `showOnboarding` props 契约未变。
- [x] AC2（R2）：事件流行渲染为四列栅格且 `channel` 仍可见；时间戳为 mono。
- [x] AC3（R3）：三档筛选控件存在且可切换；`warn` 档结果包含 `error` 级条目。
- [x] AC4（R3,R10）：计数口径由测试断言固定为「聚合后、筛选前、截断前」。构造超过 `limit` 条数据时，标题计数大于可见行数，且此差异不判为缺陷。
- [x] AC5（R4）：构造相邻重复条目时渲染出 `×N`，且聚合后计为一行。
- [x] AC6（R5）：空态显示 CTA；非空时页脚 `/monitoring` 链接可见。
- [x] AC7（R6,R7）：错误与警告行有 tint 底色、信息行无；灰度模拟下仍能区分三档。
- [x] AC8（R8）：无数据时两块各自显示空态；超长文本省略且不产生横向滚动。
- [x] AC9（R9）：本子任务改动的 CSS 中无硬编码十六进制颜色与 px 圆角字面量。
- [x] AC10（R10）：change list 与提交包含测试文件改动。
- [x] AC11：`just frontend-check-quick` 通过；中英文文案键完整。
