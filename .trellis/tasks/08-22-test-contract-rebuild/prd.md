...
- [ ] AC1 `bun run test:smoke` 退出码 0，通过测试数不少于 122。
- [ ] AC2 覆盖范围比对表落盘：迁移前后的被测组件数与被测契约条目数逐项对照，无下降项。
- [ ] AC3 19 份契约文档重写完成，`rg '\.vue|<script setup|scoped' .trellis/spec/ccr-ui/frontend/` 无匹配。
- [ ] AC4 契约断言与测试的对应表落盘，无未映射的断言。
- [ ] AC5 最小测试集在 `08-22-shell-port` 完成后 3 个工作日内可运行，含全量 IPC 命令名断言（334 base / 342 含 Windows）。
...
- [ ] AC8 `.trellis/spec/ccr-ui/frontend/index.md` 更新，反映重写后的 19 份文档结构。
