...
| [Usage Chart Stability Contracts](./usage-chart-stability-contracts.md) | ApexCharts options/series discipline, keep-mounted tab interaction, and dual-path complete CSS delivery | Complete |
| [Confirm Interaction Contracts](./confirm-interaction-contracts.md) | requestConfirm gate pattern, danger/warning semantics, no native dialogs, hook boundary | Complete |
...
- Read [Usage Chart Stability Contracts](./usage-chart-stability-contracts.md) before adding or changing any ApexCharts usage chart, its options/series wiring, or the usage tab keep-mounted structure.
...
- Read Environment-Scoped Dashboard Contracts before adding or changing a Local-only dashboard with per-environment caches or CLI version detection.
