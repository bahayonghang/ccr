...
- 适用于 `ccr-ui/src` 全部 features / shell / ui / hooks。

## 2. Signatures

- 闸门式确认(默认路径):`uiStore.requestConfirm({ title, message, confirmText?, cancelText?, type })`
  → Promise\<boolean\>,由 `src/shell/App.tsx` 挂载的 `GlobalConfirmDialog` 全局渲染 ConfirmModal。
...
- 数据 hook 内出现 `requestConfirm` 或 ConfirmModal import → 边界违约,决策上移。
...
- Bad:数据 hook 里 `if (!confirm(msg)) return` 再执行删除。
