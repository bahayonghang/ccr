...
- **动态终止升级**：Unix 对初始目标发送 TERM，然后每 300ms 全量重查约 3s；窗口内新出现的身份写入 `discovered_during_cleanup`。截止时对所有当前匹配身份发送 KILL，不限于初始 PID。Windows 上 TERM 为 `None` 时立即使用 `kill()`，但仍执行截止点复检。
...
- **最终复检**：KILL 后等待约 1s，再读取“当前匹配目标 + 全部存活身份”。仍匹配者填入 `cleanup.respawned`；新身份和无法终止的旧身份都属于进程阶段不干净。公开旧结果按 PID 投影，安全判断始终使用内部完整身份。
...
- **阶段隔离**：process cleanup、CCR runtime inspection/repair、环境提示和 doctor 分别保存结果，不得用 `?` 让 runtime 错误跳过后续 doctor。阶段错误只输出稳定状态和动作边界，不打印可能含 secret 的底层错误字符串。
...
- `cleanup.respawned` 非空或 `discovery_issue` 非空 → `std::process::exit(2)`（在 doctor 之后判定）。
...
- 固定优先级：PATH missing `127` > process remaining/unavailable `2` > runtime failure `1` > local drift `3` > success `0`。
...
- `cargo test -p ccr-codex codex_process_service -- --test-threads=1`（显式 refresh 真实 fixture、argv 分类矩阵、动态身份、signal bool、PID reuse、owner/discovery fail closed）。
...
- `cargo test -p ccr-cli --lib fix -- --test-threads=1`（highlights、process state、退出优先级）。
- `cargo test -p ccr-cli --lib codex_fix -- --test-threads=1`（`ccr codex fix` / `--dry-run` / `--repair-runtime` 解析）。
- `cargo test -p ccr --test commands codex_fix -- --test-threads=1`（binary dry-run 无写入、runtime inspection 失败仍执行 fake doctor、退出码和无 secret 输出）。
- `just lint-strict`（跨平台编译 + 无 unwrap/panic 门）。
- Assertion points: 真实 dry-run 发现 fixture PID；原始 argv sentinel 不出现在结果；app-server 命中且 exec/resume/login/任意工具不命中；失败信号不进入 `terminated`；最终存活身份不会伪报终止；dry-run 受管文件字节不变；runtime failure=1、remaining/unavailable=2、local drift=3、PATH missing=127。
