...
> Executable contract for the usage V2 typed IPC pilot: Rust DTO → generated TypeScript, committed bindings, drift guard. Extend per-domain; do not bypass.
...
- Applies to `ccr-ui/src-tauri/src/services/*`, `ccr-ui/src-tauri/src/llmusage_adapter/{queries,capabilities}.rs`, `ccr-ui/src-tauri/src/{usage_jobs,session_index_jobs}.rs`, `crates/ccr-usage/src/{queries,capabilities}.rs`, and the generated dir `ccr-ui/src/types/generated/`.
- Pilot domain: handler_registry "Usage V2" group (17 commands, all `Result<NamedDto, String>` — `Result<Value, String>` is banned in typed domains).
...
- Regeneration: `just tauri-bindings` (root) → deletes `ccr-ui/src/types/generated/`, runs `cargo test -p ccr-usage --features ts export_bindings` + `cargo test --manifest-path ccr-ui/src-tauri/Cargo.toml export_bindings`.
...
- `serde(alias)` on input DTOs → ts-rs prints "failed to parse serde attribute" during macro expansion; harmless (alias only affects deserialization; canonical snake_case is the generated shape).
