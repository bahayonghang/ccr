...
impl ConfigManager {
    /// 🏗️ 创建新的配置管理器
    ///
    /// 使用组合模式，内部初始化 ConfigFileHandler
    ...
    /// 📖 加载配置文件
    ///
    /// 委托给 ConfigFileHandler 执行实际的文件加载
    ///
    ...
    /// 💾 保存配置文件
    ///
    /// 委托给 ConfigFileHandler 执行实际的文件保存
    ///
    /// ⚠️ **并发安全**: 此方法不加锁，调用方需要在外层使用 CONFIG_LOCK 保护 RMW 序列
    ...
    /// 💾 备份配置文件
    ///
    /// 委托给 ConfigFileHandler 执行实际的文件备份
    ...
    /// 📋 列出所有配置备份文件
    ///
    /// 委托给 ConfigFileHandler 执行实际的备份列表查询
    #[allow(dead_code)]
