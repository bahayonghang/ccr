...
- **If auto-injected task context is present**: proceed with the check work.
- **If auto-injected context is missing**: use the explicit task path from the dispatch
  ...
  if present, `implement.md` if present, and `check.jsonl` (then each listed `file`).
...
3. Review all changed code against the task PRD, design/implement plan when present, and project specs.
