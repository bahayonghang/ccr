...
// ---------------------------------------------------------------------------
// Task context — prd.md, info.md, design.md, implement.md, and jsonl files
// ---------------------------------------------------------------------------
...
function buildTaskContext(projectRoot: string, taskDir: string, agentType?: AgentType): string {
   ...
   // prd.md, info.md, design.md, and implement.md — included when present
   let prd = "";
   ...
   let design = "";
   try { design = readFileSync(join(taskDir, "design.md"), "utf-8"); } catch { }
   if (design.trim()) parts.push(`## Design\n\n${design.trim()}`);

   let implement = "";
   try { implement = readFileSync(join(taskDir, "implement.md"), "utf-8"); } catch { }
   if (implement.trim()) parts.push(`## Implement\n\n${implement.trim()}`);
   ...
   if (agentType === "trellis-implement") {
      ...
      jsonlNames = []; // research agent gets task markdown only, not jsonl
   } else {
