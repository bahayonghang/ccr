...
**原因：** 可能在使用旧版本 CCR，会同步 `ccr-ui/`、`backups/`、`history/` 等目录
...
- 同步只执行当前 CLI 帮助中暴露的 config、folder、all 与快捷目录路径。
- 冲突处理、自动同步和版本历史不是当前命令契约；脚本不要假定这些能力存在。
- 对覆盖性 pull/push 使用预览、备份和显式确认选项。
