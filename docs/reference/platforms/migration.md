...
This page describes the migration patterns that are still supported by the current CCR command surface.

## Current Rules

- there is no documented `ccr migrate`
- there is no `ccr platform migrate`
- migration is done by combining `platform init`, `export`, `import`, and manual token replacement
...
## Pattern 3: Legacy single-file config to current layout

If you still have `~/.ccs_config.toml`, treat it as a reference source instead of expecting a dedicated migration command.
...
- imported or recreated profiles appear in `ccr list`
...
- rely on the automatically created backup when using `ccr import --backup` or destructive settings writes
