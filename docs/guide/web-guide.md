...
> ⚠️ 说明：本指南介绍的是 `ccr web` 提供的轻量级旧版 Web 界面和 REST API。该命令主要用于兼容与编程访问。
> 
> **新的网页端推荐使用 `ccr ui`（CCR UI 全栈应用）作为主要 Web 界面，`ccr web` 将在未来逐步弃用。**
...
- **入口**：配置列表顶部、排序控件旁的“🏢 提供商统计”按钮，点击弹出模态框
- **数据源**：调用 `/api/stats/provider-usage`，从 `~/.ccr/platforms/claude/profiles.toml` 汇总 provider → usage_count
- **展示**：provider → 调用次数的列表 + 水平柱状图，按次数降序；空数据显示友好提示
- **使用步骤**：
  1. 点击“提供商统计”按钮
  2. 查看各 provider/platform（如 claude、codex、gemini、qwen、unknown）的调用占比，用于路由或限额调整
