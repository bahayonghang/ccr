...
| 选项 | 简写 | 说明 |
|------|------|------|
| `--yes` | -y | 启动时启用自动确认模式（原 YOLO 模式），跳过危险操作确认 |
...
::: warning 危险操作警告
自动确认模式会跳过确认，使用时请务必小心！特别是在删除配置时。
:::

### 🎨 视觉与交互特性（已优化）

**统一主题配色（theme.rs）：**
- 绿色（Success）：成功状态、当前配置
- 青色（Accent）：标签与说明文字
- 红色（Error）：错误消息
- 黄色（Highlight）：警告/选中项/强调
- 深灰（Secondary）：提示与次要说明
...
- **增量渲染**：仅在按键事件或需要状态更新时重绘，减少不必要的 draw
- **节流控制**：统一 tick 间隔（250ms），消息帧计数在 tick 上递减
- **渲染缓存**：在进入 TUI 时与标签切换时刷新缓存，渲染阶段不进行磁盘 I/O
- **并发安全**：继承 CCR 核心的文件锁与原子写入机制
