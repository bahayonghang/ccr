...
description: |
  ...
  and self-fixes issues. On Kimi Code the main session uses this repo's pull
  workflow and dispatches the built-in coder sub-agent with these
  instructions; the first prompt line must be Active task: <path>.
---

## Required: Load Trellis Context First

This repository does NOT auto-inject task context via hook (official Kimi hooks exist; this repo uses pull). Before doing anything else, you MUST load context yourself.
...
Official Kimi Code supports project-level custom agents and hooks. **This repository** does not install them. The main session still uses the pull workflow and dispatches the built-in `coder` sub-agent via the Agent tool with a prompt that:
...
This repo does not auto-inject SessionStart task context. Always pull context as required below.
