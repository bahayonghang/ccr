...
description: |
  ...
  main session uses this repo's pull workflow and dispatches the built-in
  coder sub-agent with these instructions because it is the only built-in
  sub-agent with file-editing tools.
---
# Research Agent
...
Official Kimi Code supports project-level custom agents and hooks. **This repository** does not install them. The built-in `explore` sub-agent is read-only, so it cannot satisfy this agent's persistence contract. The main session still uses the pull workflow and dispatches the built-in `coder` sub-agent via the Agent tool with a prompt that:
