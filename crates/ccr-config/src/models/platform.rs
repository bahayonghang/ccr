...
use ccr_core::utils::AutoCompletable;
...
impl FromStr for Platform {
    type Err = ccr_core::core::error::CcrError;
...
impl PlatformPaths {
    ...
    fn get_ccr_root() -> Result<PathBuf> {
        if let Ok(custom_root) = std::env::var("CCR_ROOT") {
            ...
            let home = dirs::home_dir().ok_or_else(|| {
                ccr_core::core::error::CcrError::ConfigError("无法获取用户主目录".into())
            })?;
            Ok(home.join(".ccr"))
        }
    }
    ...
    pub fn ensure_directories(&self) -> Result<()> {
        ...
        let history_parent = self.history_file.parent().ok_or_else(|| {
            ccr_core::core::error::CcrError::ConfigError(format!(
                "无法获取 history 文件的父目录: {:?}",
                self.history_file
            ))
        })?;
