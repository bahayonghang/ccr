...
pub struct CodexAuthApp {
    ...
    /// 后台任务消息发送端
    task_tx: UnboundedSender<CodexAuthTaskMessage>,
    /// 后台任务消息接收端
    task_rx: UnboundedReceiver<CodexAuthTaskMessage>,
    /// Usage 后台任务是否进行中
...
impl CodexAuthApp {
    ...
    pub fn with_task_executor(task_executor: AsyncTaskExecutor) -> Result<Self> {
        Self::from_service_with_task_executor(CodexAuthService::new()?, task_executor)
    }
    ...
    fn start_preview_prefetch(&mut self, force_refresh: bool) -> bool {
        ...
        self.task_executor.spawn(async move {
            let quotas = match CodexQuotaService::new() {
                Ok(service) => {
                    if force_refresh {
                        service
            ...
            let _ = tx.send(CodexAuthTaskMessage::Preview(quotas));
        });
...
mod tests {
    ...
    fn on_activated_waits_one_second_before_starting_usage_and_preview_fetch() {
        ...
        assert!(app.on_tick());
        assert!(
            app.usage_task_active || !matches!(app.usage_state, UsageState::Loading),
            "usage fetch should have started or completed"
        );
        assert!(
            app.preview_task_active || !app.preview_cache.is_empty(),
