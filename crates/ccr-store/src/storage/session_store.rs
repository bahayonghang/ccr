...
fn parse_platform_safe(s: &str) -> Platform {
    ...
    pub fn list(&self, filter: SessionFilter) -> Result<Vec<SessionSummary>> {
        ...
        let rows = stmt
            .query_map(param_refs.as_slice(), |row| {
                Ok(SessionSummary {
                    id: row.get(0)?,
                    platform: parse_platform_safe(&row.get::<_, String>(1)?),
                    title: row.get(2)?,
                    cwd: row.get(3)?,
...
mod tests {
    ...
    #[test]
    fn test_roundtrip_droid_platform() {
        let db = create_test_db();
        let store = SessionStore::new(&db);

        let sessions = vec![create_test_session("droid-1", Platform::Droid)];
        store.upsert_sessions(&sessions).unwrap();

        let session = store.get("droid-1").unwrap().unwrap();
        assert_eq!(session.platform, Platform::Droid);

        let summaries = store.list(SessionFilter::default()).unwrap();
        assert_eq!(summaries.len(), 1);
        assert_eq!(summaries[0].platform, Platform::Droid);
    }
}
