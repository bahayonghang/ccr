...
impl Cli {
    ...
    /// `ccr codex` / `ccr grok auth` 无嵌套动作也视为 TUI 路由模式
    #[cfg(feature = "tui")]
    pub fn is_tui_mode(&self) -> bool {
        ...
        matches!(
            self.command,
            Some(Commands::Codex { action: None })
                | Some(Commands::Grok {
                    action: Some(super::subcommands::GrokAction::Auth { action: None })
                })
        )
    }
}
...
mod tests {
    ...
    #[test]
    fn codex_fix_doctor_parses() {
        let cli = Cli::try_parse_from(["ccr", "codex", "fix", "--doctor"]).unwrap();
        match cli.command {
            Some(Commands::Codex {
                action:
                    Some(CodexAction::Fix {
    ...
    #[test]
    fn codex_fix_doctor_parses_with_dry_run() {
        let cli = Cli::try_parse_from(["ccr", "codex", "fix", "--dry-run", "--doctor"]).unwrap();
        match cli.command {
            Some(Commands::Codex {
                action:
                    Some(CodexAction::Fix {
    ...
    #[test]
    fn codex_fix_repair_runtime_does_not_imply_doctor() {
        let cli = Cli::try_parse_from(["ccr", "codex", "fix", "--repair-runtime"]).unwrap();
        match cli.command {
            Some(Commands::Codex {
                action:
                    Some(CodexAction::Fix {
