...
impl ExportCrypto {
    ...
    pub fn encrypt_export(
        ...
        let salt_bytes: &[u8; SALT_SIZE] = salt.as_ref();
        let nonce_bytes: &[u8; NONCE_SIZE] = nonce.as_ref();

        // 派生密钥
        let key = Self::derive_key(password, salt_bytes, &kdf_params)?;
    ...
    pub fn decrypt_export(encrypted: &CodexAuthEncryptedExport, password: &str) -> Result<String> {
        ...
        let nonce = Nonce::try_from(nonce_bytes.as_slice())
            .map_err(|_| CcrError::ConfigError("Nonce 长度无效".to_string()))?;
...
mod tests {
    ...
    fn test_encrypt_decrypt_roundtrip() {
        ...
        assert_eq!(
            BASE64.decode(&encrypted.encryption.salt).unwrap().len(),
            SALT_SIZE
        );
        assert_eq!(
            BASE64.decode(&encrypted.encryption.nonce).unwrap().len(),
            NONCE_SIZE
        );
        assert_eq!(
            BASE64.decode(&encrypted.encrypted_payload).unwrap().len(),
            accounts_json.len() + 16
        );
    ...
    fn decrypts_aes_gcm_010_fixture() {
        ...
        let encrypted: CodexAuthEncryptedExport = serde_json::from_str(
            r#"{"version":"2.0","format":"encrypted","exported_at":"2026-08-30T12:34:56Z","account_count":1,"encryption":{"algorithm":"aes-256-gcm","kdf":"argon2id","kdf_params":{"m_cost":65536,"t_cost":3,"p_cost":1},"salt":"S5ENlysejQBBfPFNcEs7u1DvMBZYJe71NKufuBTVCec=","nonce":"zih1f1iS1Kfl52ew"},"encrypted_payload":"8OXQwXAusczU2peBPt2B5qdUiBJWiIXauHvKN8dNXWJCc6XpWdPPvEbRK4rsrkixPNDzexC6XuIW1VpWASo="}"#,
        )
        .unwrap();
        let decrypted = ExportCrypto::decrypt_export(&encrypted, "legacy-password").unwrap();
        assert_eq!(
            decrypted,
            r#"{"legacy":"codex-auth","account_id":"acc-010"}"#
        );
    }
    ...
    fn derives_same_key_as_argon2_053_fixture() {
        ...
        let key = ExportCrypto::derive_key("argon2-migration-fixture", &salt, &params).unwrap();
    ...
    #[test]
    fn rejects_invalid_argon2_params_and_salt() {
        let salt = [0x5a; SALT_SIZE];
        ...
        let invalid_params_error =
            ExportCrypto::derive_key("password", &salt, &invalid_params).unwrap_err();
        assert!(invalid_params_error.to_string().contains("Argon2 参数无效"));

        let invalid_salt_error =
            ExportCrypto::derive_key("password", &[], &KdfParams::default()).unwrap_err();
        assert!(invalid_salt_error.to_string().contains("密钥派生失败"));
    }

    #[test]
    fn rejects_invalid_nonce_and_tampered_nonce_ciphertext_or_tag() {
        let mut encrypted = ExportCrypto::encrypt_export(
            r#"{"test":"boundaries"}"#,
            "correct-password",
            Utc::now(),
            1,
        ...
        let original_nonce = BASE64.decode(&encrypted.encryption.nonce).unwrap();
        encrypted.encryption.nonce = BASE64.encode(&original_nonce[..NONCE_SIZE - 1]);
        assert!(ExportCrypto::decrypt_export(&encrypted, "correct-password").is_err());

        encrypted.encryption.nonce = BASE64.encode(&original_nonce);
        let mut tampered_nonce = original_nonce.clone();
        tampered_nonce[0] ^= 0x01;
        encrypted.encryption.nonce = BASE64.encode(tampered_nonce);
        assert!(ExportCrypto::decrypt_export(&encrypted, "correct-password").is_err());

        encrypted.encryption.nonce = BASE64.encode(original_nonce);
        let ciphertext = BASE64.decode(&encrypted.encrypted_payload).unwrap();
        for index in [0, ciphertext.len() - 1] {
            let mut tampered = ciphertext.clone();
            tampered[index] ^= 0x01;
            encrypted.encrypted_payload = BASE64.encode(tampered);
            assert!(ExportCrypto::decrypt_export(&encrypted, "correct-password").is_err());
        }
    }
}
