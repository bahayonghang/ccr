...
pub struct DroidConfigManager {
    config_path: PathBuf,
}

impl DroidConfigManager {
    /// 创建默认实例，使用项目根目录的 .factory/settings.json
    pub fn default() -> Result<Self, String> {
        let current_dir =
            std::env::current_dir().map_err(|e| format!("无法获取当前目录: {}", e))?;
        let factory_dir = current_dir.join(".factory");
        let config_path = factory_dir.join("settings.json");
    ...
    /// 读取配置文件
    pub fn read_config(&self) -> Result<DroidConfig, String> {
        if !self.config_path.exists() {
            info!("Droid 配置文件不存在，返回空配置");
            return Ok(DroidConfig::default());
        }

        let content = fs::read_to_string(&self.config_path)
            .map_err(|e| format!("读取 Droid 配置文件失败: {}", e))?;

        serde_json::from_str(&content).map_err(|e| format!("解析 Droid JSON 失败: {}", e))
    }

    /// 写入配置文件
    pub fn write_config(&self, config: &DroidConfig) -> Result<(), String> {
        let json_str = serde_json::to_string_pretty(config)
            .map_err(|e| format!("序列化 Droid 配置失败: {}", e))?;

        self.atomic_write(&self.config_path, &json_str)?;
        info!("Droid 配置已写入: {:?}", self.config_path);
        Ok(())
    }
    ...
    /// 列出所有 MCP 服务器
    pub fn list_mcp_servers(&self) -> Result<Vec<(String, DroidMcpServer)>, String> {
    ...
    /// 添加 MCP 服务器
    pub fn add_mcp_server(&self, name: String, server: DroidMcpServer) -> Result<(), String> {
        ...
        info!("已添加 Droid MCP 服务器: {}", name);
        Ok(())
    }

    /// 更新 MCP 服务器
    pub fn update_mcp_server(&self, name: &str, server: DroidMcpServer) -> Result<(), String> {
        let mut config = self.read_config()?;
    ...
    /// 获取完整配置
    pub fn get_config(&self) -> Result<DroidConfig, String> {
        self.read_config()
    }

    /// 更新完整配置
    pub fn update_config(&self, config: &DroidConfig) -> Result<(), String> {
        self.write_config(config)?;
        info!("已更新 Droid 完整配置");
        Ok(())
    }
...
mod tests {
    ...
    fn test_read_write_config() {
        ...
        let manager = DroidConfigManager::with_path(config_path.clone());
    ...
    fn test_mcp_server_crud() {
        let temp_dir = TempDir::new().unwrap();
        let config_path = temp_dir.path().join("settings.json");
        let manager = DroidConfigManager::with_path(config_path);
