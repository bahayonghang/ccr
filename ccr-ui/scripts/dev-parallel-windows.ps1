...
$backendJob = Start-Job -ScriptBlock {
    ...
    # Run cargo and log output
    $logPath = "$workDir/logs/backend-console.log"
    cargo run 2>&1 | Tee-Object -FilePath $logPath -Append
...
$maxWait = 90
Write-Host "[Backend] Waiting for health check (http://127.0.0.1:38081/health)..." -ForegroundColor Cyan
...
for ($i = 0; $i -lt $maxWait; $i++) {
    ...
    # Health check
    try {
        $response = Invoke-WebRequest -UseBasicParsing -TimeoutSec 2 -Uri 'http://127.0.0.1:38081/health' -ErrorAction SilentlyContinue
...
try {
    ...
    # Frontend runs in foreground with live output, also writes to log
    ...
    cmd /c "bun run dev 2>&1 || exit 0" | Tee-Object -FilePath "$RootDir/logs/frontend.log" -Append
} finally {
