...
<template>
  <aside
    class="cp-rail"
    :aria-label="$t('claudeProfiles.contextRail.ariaLabel')"
  >
    <!-- ========================================================
         面板 1：当前 Profile 详情
         ======================================================== -->
    ...
    >
      <header class="cp-rail-card__head">
        ...
        >
          {{ $t('claudeProfiles.contextRail.activeTitle') }}
        </h3>
      </header>
      ...
      >
        ...
        <dl class="cp-rail-fields">
          <div class="cp-rail-field">
            <dt class="cp-rail-field__label">
              {{ $t('claudeProfiles.fields.baseUrl') }}
            </dt>
          ...
          <div class="cp-rail-field">
            <dt class="cp-rail-field__label">
              {{ $t('claudeProfiles.fields.authMode') }}
            </dt>
        ...
        >
          <SIcon
            ...
            size="w-3.5 h-3.5"
          />
          <span>{{ $t('claudeProfiles.contextRail.editAction') }}</span>
        </button>
      </div>
      ...
      >
        ...
        <div class="cp-rail-empty__title">
          {{ $t('claudeProfiles.contextRail.activeEmpty') }}
        </div>
        <div class="cp-rail-empty__hint">
          {{ $t('claudeProfiles.contextRail.activeEmptyHint') }}
        </div>
      </div>
    </section>
    ...
    >
      <header class="cp-rail-card__head">
        ...
        <h3
          ...
          class="cp-rail-card__title"
        >
          {{ $t('claudeProfiles.contextRail.distributionTitle') }}
        </h3>
      </header>
      ...
      <div class="cp-rail-section">
        <div class="cp-rail-section__head">
          {{ $t('claudeProfiles.contextRail.providerSection') }}
        </div>
        <ul
            ...
            <div class="cp-rail-bar__label">
              {{
                item.provider === 'Unknown'
                  ? $t('claudeProfiles.contextRail.unknownProvider')
                  : item.provider
              }}
            </div>
      ...
      <div class="cp-rail-section">
        <div class="cp-rail-section__head">
          {{ $t('claudeProfiles.contextRail.authSection') }}
        </div>
        <ul
          class="cp-rail-bars"
          role="presentation"
      ...
      <div class="cp-rail-section">
        <div class="cp-rail-section__head">
          {{ $t('claudeProfiles.contextRail.tagsSection') }}
        </div>
        <div
        ...
        <div
          ...
          class="cp-rail-section__empty"
        >
          {{ $t('claudeProfiles.contextRail.noTags') }}
        </div>
      </div>
    </section>
    ...
    >
      <header class="cp-rail-card__head">
        ...
        <h3
          ...
          class="cp-rail-card__title"
        >
          {{ $t('claudeProfiles.contextRail.auditTitle') }}
        </h3>
        <span
      ...
      >
        <SIcon
          ...
          size="w-4 h-4"
        />
        <span>{{ $t('claudeProfiles.contextRail.auditClean') }}</span>
      </div>
        ...
        >
          <div class="cp-rail-issue-group__head">
            ...
            <span>
              {{
                tf(
                  'claudeProfiles.contextRail.issues.duplicateRuntime',
...
import {
  useClaudeProfilesInsights,
  type ClaudeMissingField,
} from '@/composables/useClaudeProfilesInsights'
...
const {
  ...
  totalIssueCount,
} = useClaudeProfilesInsights(profilesRef)

const activeHeadingId = 'claude-rail-active-heading'
const distributionHeadingId = 'claude-rail-distribution-heading'
const auditHeadingId = 'claude-rail-audit-heading'

const EM_DASH = '—'

const authModeLabel = (mode: ClaudeProfileAuthMode): string =>
  mode === 'api_key'
...
const activeModelText = computed(() => {
  ...
  return model || EM_DASH
})

const activeAuthModeLabel = computed(() =>
  authModeLabel(props.activeProfile?.auth_mode ?? 'subscription'),
)

const activeTags = computed(() => props.activeProfile?.tags ?? [])

const formatMissingMessage = (missing: ClaudeMissingField[]): string => {
  const parts = missing.map((field) => {
    if (field === 'base_url') return t('claudeProfiles.contextRail.issues.missingBaseUrl')
    if (field === 'model') return t('claudeProfiles.contextRail.issues.missingModel')
    return t('claudeProfiles.contextRail.issues.missingAccount')
  })
  return parts.join(' · ')
}

const runtimeSummary = (profile: ClaudeProfile): string => {
  const model = profile.model?.trim()
    || profile.default_sonnet_model?.trim()
    || profile.default_opus_model?.trim()
    || EM_DASH
  const baseUrl = profile.base_url?.trim() || EM_DASH
