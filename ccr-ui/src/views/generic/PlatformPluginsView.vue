<template>
  <PageShell class="platform-plugins-view">
    <template #header>
      <PageHeader :title="$t(`${i18nPrefix}.title`)">
        <template #leading>
          <SIcon
            name="Puzzle"
            size="w-6 h-6"
        ...
        <template #status>
          <span class="px-3 py-1 rounded-md text-sm font-medium bg-bg-elevated text-text-secondary border border-border-default">{{ plugins.length }}</span>
        </template>
        <template #actions>
          <RouterLink
            :to="parentPath"
