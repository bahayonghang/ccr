            ...
            >
              ...
              >
                <button
                  ...
                  type="button"
                  role="radio"
                  class="app-settings-group__item"
                  :class="{ 'app-settings-group__item--active': theme === option.value }"
                  :data-testid="`settings-theme-${option.value}`"
                  :aria-checked="theme === option.value"
                  :aria-pressed="theme === option.value"
                  @click="setTheme(option.value)"
                >
                  <span class="app-settings-group__icon">
                    <SIcon
                      :name="option.icon"
                      size="w-4 h-4"
                    />
                  </span>
                  <span class="app-settings-group__copy">
                    <span class="app-settings-group__title">{{ option.label }}</span>
                    <span class="app-settings-group__caption">{{ option.description }}</span>
                  </span>
                </button>
              </div>
              <p
                v-if="theme === 'system'"
                class="app-settings-group__resolved"
              >
                {{ systemResolvedHint }}
              </p>

              <div class="app-settings-card__header app-settings-card__header--follow">
                <div>
                  <p class="app-settings-card__eyebrow">
...
import { type FlavorMode, type ThemeMode } from '@/utils/themeBootstrap'
...
// 色板预览 hex 白名单：静态复制自 tokens.css 的 surface 子集。
...
.app-settings-nav__button--active .app-settings-nav__icon {
  border-color: rgb(var(--color-accent-primary-rgb) / 24%);
...
.app-settings-card__header--follow {
  @apply mt-6;
}
...
/* --- 主题分组列表：light / dark / system --- */
.app-settings-group {
  @apply mt-4 flex min-w-0 flex-col overflow-hidden rounded-lg border;

  border-color: var(--color-border-default);
  background: var(--color-bg-base);
}

.app-settings-group__item {
  @apply flex min-w-0 items-start gap-3 border-b border-transparent px-3.5 py-3 text-left last:border-b-0;

  border-color: var(--color-border-subtle);
  color: var(--color-text-secondary);
}

.app-settings-group__item:last-child {
  border-bottom-color: transparent;
}

.app-settings-group__item:hover {
  background: var(--color-bg-elevated);
  color: var(--color-text-primary);
}

.app-settings-group__item--active {
  background: rgb(var(--color-accent-primary-rgb) / 10%);
  color: var(--color-text-primary);
}

.app-settings-group__icon {
...
.app-settings-group__item--active .app-settings-group__icon {
  border-color: rgb(var(--color-accent-primary-rgb) / 24%);
  color: var(--color-text-primary);
}

.app-settings-group__copy {
  @apply min-w-0;
}

.app-settings-group__title {
  @apply block text-sm font-semibold;

  color: var(--color-text-primary);
}

.app-settings-group__caption {
  @apply mt-0.5 block text-xs leading-5;

  color: var(--color-text-muted);
}

.app-settings-group__resolved {
...
.app-settings-flavor-grid {
  @apply mt-4 grid min-w-0 grid-cols-1 gap-3 md:grid-cols-2;
}
