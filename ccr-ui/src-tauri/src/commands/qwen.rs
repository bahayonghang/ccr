//! Qwen 命令 — Settings/MCP/Slash Commands。
//!
//! Config file: ~/.qwen/settings.json  (or project-root/.qwen/settings.json)
//! Slash commands: ~/.qwen/commands/*.toml  (user-level)
//!                 {project_root}/.qwen/commands/*.toml  (project-level)
...
/// 定位 Qwen settings.json（项目级优先，否则用户级）
fn qwen_config_path() -> Result<PathBuf, String> {
    // 尝试项目级
    ...
    let proj_dir = project_root.join(".qwen");
    let proj_cfg = proj_dir.join("settings.json");
    if proj_dir.exists() || proj_cfg.exists() {
        if !proj_dir.exists() {
            std::fs::create_dir_all(&proj_dir).map_err(|e| format!("创建 .qwen 目录失败: {e}"))?;
        }
        return Ok(proj_cfg);
    }
    // 用户级
    ...
    if !user_dir.exists() {
        std::fs::create_dir_all(&user_dir).map_err(|e| format!("创建 .qwen 目录失败: {e}"))?;
    }
    Ok(user_dir.join("settings.json"))
}

/// 读取 Qwen settings.json，不存在时返回空对象
fn read_qwen_config() -> Result<Value, String> {
    let path = qwen_config_path()?;
    if !path.exists() {
        return Ok(json!({}));
    }
    let content =
        std::fs::read_to_string(&path).map_err(|e| format!("读取 Qwen 配置文件失败: {e}"))?;
    serde_json::from_str(&content).map_err(|e| format!("解析 Qwen JSON 失败: {e}"))
}

/// 原子写入 Qwen settings.json
fn write_qwen_config(config: &Value) -> Result<(), String> {
    let path = qwen_config_path()?;
    let parent = path.parent().ok_or_else(|| "无法获取父目录".to_string())?;
    let json_str =
        serde_json::to_string_pretty(config).map_err(|e| format!("序列化 Qwen 配置失败: {e}"))?;
    let mut tmp =
...
fn qwen_commands_dirs() -> Result<(PathBuf, PathBuf), String> {
    let project = find_project_root().join(".qwen").join("commands");
    let home = dirs::home_dir().ok_or_else(|| "无法获取用户主目录".to_string())?;
...
#[tauri::command]
pub async fn qwen_get_settings() -> Result<Value, String> {
    tokio::task::spawn_blocking(read_qwen_config)
        .await
        .map_err(|e| format!("任务执行失败: {e}"))?
}

#[tauri::command]
pub async fn qwen_update_settings(settings: Value) -> Result<Value, String> {
    tokio::task::spawn_blocking(move || {
        write_qwen_config(&settings)?;
        Ok(json!({ "message": "Qwen 配置更新成功" }))
    })
    .await
    .map_err(|e| format!("任务执行失败: {e}"))?
}
...
#[tauri::command]
pub async fn qwen_list_mcp_servers() -> Result<Value, String> {
    tokio::task::spawn_blocking(|| {
        let config = read_qwen_config()?;
        let servers = config
            .get("mcpServers")
        ...
        let list: Vec<Value> = servers
            ...
            .map(|(name, server)| {
                ...
                // 计算传输类型
                let transport = if obj.contains_key("httpUrl") {
                    "http"
                ...
                obj.insert("transportType".to_string(), json!(transport));
                Value::Object(obj)
            })
            .collect();
        Ok(json!(list))
...
#[tauri::command]
pub async fn qwen_add_mcp_server(name: String, config: Value) -> Result<Value, String> {
    tokio::task::spawn_blocking(move || {
        let mut full = read_qwen_config()?;
        let servers = full
            .as_object_mut()
...
#[tauri::command]
pub async fn qwen_update_mcp_server(name: String, config: Value) -> Result<Value, String> {
    tokio::task::spawn_blocking(move || {
        let mut full = read_qwen_config()?;
        let servers = full
            .as_object_mut()
...
#[tauri::command]
pub async fn qwen_delete_mcp_server(name: String) -> Result<String, String> {
    tokio::task::spawn_blocking(move || {
        let mut full = read_qwen_config()?;
        let servers = full
            .as_object_mut()
...
#[tauri::command]
pub async fn qwen_list_slash_commands() -> Result<Value, String> {
    tokio::task::spawn_blocking(|| {
        let (proj, user) = qwen_commands_dirs()?;
...
#[tauri::command]
pub async fn qwen_add_slash_command(name: String, config: Value) -> Result<Value, String> {
    tokio::task::spawn_blocking(move || {
        let (proj, _user) = qwen_commands_dirs()?;
        let folder = config
            .get("folder")
            .and_then(|v| v.as_str())
            .unwrap_or("")
...
#[tauri::command]
pub async fn qwen_update_slash_command(name: String, config: Value) -> Result<Value, String> {
    tokio::task::spawn_blocking(move || {
        let (proj, user) = qwen_commands_dirs()?;
...
#[tauri::command]
pub async fn qwen_delete_slash_command(name: String) -> Result<String, String> {
    tokio::task::spawn_blocking(move || {
        let (proj, user) = qwen_commands_dirs()?;
        let target = find_command_file(&proj, &user, &name)?
            .ok_or_else(|| format!("斜杠命令 '{name}' 不存在"))?;
