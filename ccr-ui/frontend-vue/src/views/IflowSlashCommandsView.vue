          ...
          <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-4">
              <h2 class="text-2xl font-bold" :style="{ color: 'var(--text-primary)' }">
                <Command class="inline-block w-7 h-7 mr-2" />iFlow Slash Commands 管理
              </h2>
...
const handleDelete = async (name: string) => {
  ...
  try { await deleteIflowSlashCommand(name); loadCommands() } catch (err) { console.error('删除失败:', err); alert('删除失败') }
}

const handleToggle = async (name: string) => {
  try { await toggleIflowSlashCommand(name); loadCommands() } catch (err) { console.error('切换状态失败:', err); alert('切换状态失败') }
}
