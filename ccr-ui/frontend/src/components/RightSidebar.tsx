...
export default function RightSidebar({ configs, currentFilter, onConfigClick }: RightSidebarProps) {
  ...
  return (
    <aside
      className="rounded-xl p-5 h-fit sticky top-5 max-h-[calc(100vh-120px)] overflow-y-auto"
      style={{
          ...
          filtered.map((config) => (
            <li key={config.name}>
              <button
                ...
                className="w-full text-left px-3 py-2 rounded-lg text-sm font-medium transition-all relative overflow-hidden group"
                style={{
