...
export const updateIflowMcpServer = async (
  _name: string,
  _request: PlatformMcpServerRequest
): Promise<string> => {
  const response = await api.put<ApiResponse<string>>(
    `/iflow/mcp/${encodeURIComponent(_name)}`,
    _request
  )
  return response.data.message || 'OK'
}
...
export const listIflowAgents = async (): Promise<PlatformAgentsResponse> => {
  ...
  return response.data.data ?? { agents: [], folders: [] }
}
...
export const toggleIflowAgent = async (_name: string): Promise<string> => {
  const response = await api.put<ApiResponse<string>>(
    `/iflow/agents/${encodeURIComponent(_name)}/toggle`
  )
  return response.data.message || 'OK'
}
...
export const listIflowSlashCommands = async (): Promise<PlatformSlashCommandsResponse> => {
  const response =
    await api.get<ApiResponse<PlatformSlashCommandsResponse>>('/iflow/slash-commands')
  return response.data.data ?? { commands: [], folders: [] }
}

export const addIflowSlashCommand = async (
  _request: PlatformSlashCommandRequest
): Promise<string> => {
  const response = await api.post<ApiResponse<string>>('/iflow/slash-commands', _request)
  return response.data.message || 'OK'
}

export const updateIflowSlashCommand = async (
  _name: string,
  _request: PlatformSlashCommandRequest
): Promise<string> => {
  const response = await api.put<ApiResponse<string>>(
    `/iflow/slash-commands/${encodeURIComponent(_name)}`,
    _request
  )
  return response.data.message || 'OK'
}
...
export const toggleIflowSlashCommand = async (_name: string): Promise<string> => {
  const response = await api.put<ApiResponse<string>>(
    `/iflow/slash-commands/${encodeURIComponent(_name)}/toggle`
  )
  return response.data.message || 'OK'
}
...
export const updateIflowPlugin = async (
  _id: string,
  _request: PlatformPluginRequest
): Promise<string> => {
  throw new Error('Backend API not implemented yet')
}
