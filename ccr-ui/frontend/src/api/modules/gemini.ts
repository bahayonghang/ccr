...
export const deleteGeminiMcpServer = async (name: string): Promise<string> => {
  const response = await api.delete<ApiResponse<string>>(`/gemini/mcp/${encodeURIComponent(name)}`)
  return response.data.data!
}
...
export const listGeminiSlashCommands = async (): Promise<PlatformSlashCommandsResponse> => {
  const response =
    await api.get<ApiResponse<PlatformSlashCommandsResponse>>('/gemini/slash-commands')
...
export const updateGeminiSlashCommand = async (
  ...
  const response = await api.put<ApiResponse<string>>(
    `/gemini/slash-commands/${encodeURIComponent(_name)}`,
    _request
  )
  return response.data.message || 'OK'
}
...
export const toggleGeminiSlashCommand = async (_name: string): Promise<string> => {
  const response = await api.put<ApiResponse<string>>(
    `/gemini/slash-commands/${encodeURIComponent(_name)}/toggle`
  )
  return response.data.message || 'OK'
}
