    ...
    <div style="display: flex; gap: 0">
      ...
      <div :style="{ width: '180px', background: 'var(--bg-secondary)', borderRight: '1px solid var(--border-color)', padding: '12px 8px', overflowY: 'auto', height: 'calc(100vh - 40px)', flexShrink: 0 }">
        <h4 :style="{ color: 'var(--text-muted)', fontSize: '11px', fontWeight: '600', marginBottom: '8px', marginLeft: '8px', textTransform: 'uppercase', letterSpacing: '0.5px' }">
          {{ $t('codex.slashCommands.folders.label') }}
        </h4>
        <div
        ...
        <div class="max-w-[1600px] mx-auto">
          <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-4">
              ...
              >
                <Command class="inline-block w-7 h-7 mr-2" />{{ $t('codex.slashCommands.pageTitle') }}
              </h2>
              <span
            ...
            <div class="flex items-center gap-3">
              ...
              >
                <Home class="w-4 h-4" /><span>{{ $t('codex.slashCommands.backToHome') }}</span>
              </RouterLink>
              <button
                ...
                @click="handleAdd"
              >
                <Plus class="inline-block w-5 h-5 mr-2" />{{ $t('codex.slashCommands.addCommandShort') }}
              </button>
            </div>
          </div>
          ...
          <div class="mb-4">
            <div class="relative">
              ...
              <input
                v-model="searchQuery"
                type="text"
                :placeholder="$t('codex.slashCommands.searchPlaceholder')"
            ...
            <p
              ...
              class="mt-2 text-sm"
              :style="{ color: 'var(--text-muted)' }"
              v-html="$t('codex.slashCommands.searchResults', { count: filteredCommands.length })"
            />
          </div>
          ...
          <div class="space-y-4">
            ...
            <div
              ...
              :style="{ color: 'var(--text-muted)' }"
            >
              {{ $t('codex.slashCommands.emptyState') }}
            </div>
            <div
              ...
              :style="{ color: 'var(--text-muted)' }"
            >
              <Search class="w-12 h-12 mx-auto mb-3 opacity-50" /><p>{{ $t('codex.slashCommands.noResults') }}</p><p class="text-sm mt-2">
                {{ $t('codex.slashCommands.noResultsHint') }}
              </p>
            </div>
            <div
              ...
              <div class="flex items-start justify-between">
                <div class="flex-1">
                  <div class="flex items-center gap-3 mb-2">
                    ...
                    >{{ $t('codex.slashCommands.disabledBadge') }}</span>
                  </div>
                  <p
                    class="mb-2"
                ...
                <div class="flex gap-2 ml-4">
                  <button
                    ...
                    :title="cmd.disabled ? $t('codex.slashCommands.enable') : $t('codex.slashCommands.disable')"
    ...
    >
      ...
      >
        <h3
          ...
          :style="{ color: 'var(--text-primary)' }"
        >
          {{ editingCommand ? $t('codex.slashCommands.editCommandShort') : $t('codex.slashCommands.addCommandShort') }}
        </h3>
        <div class="space-y-4">
          <div>
            <label
            ...
            >{{ $t('codex.slashCommands.nameLabel') }}</label>
            <input
          ...
          <div>
            <label
              class="block mb-2 font-medium"
              :style="{ color: 'var(--text-secondary)' }"
            >{{ $t('codex.slashCommands.commandLabel') }}</label>
            <input
          ...
          <div>
            <label
              class="block mb-2 font-medium"
              :style="{ color: 'var(--text-secondary)' }"
            >{{ $t('codex.slashCommands.descriptionLabel') }}</label>
            <textarea
        ...
        <div class="flex gap-3 mt-6">
          <button
            ...
            @click="handleSubmit"
          >
            {{ editingCommand ? $t('codex.slashCommands.save') : $t('codex.slashCommands.add') }}
          </button>
          <button
            class="flex-1 px-6 py-3 rounded-lg font-medium transition-all hover:scale-105"
...
const folderOptions = computed(() => [
  { value: '', label: t('codex.slashCommands.folders.all'), icon: Folder, count: stats.value.total },
  { value: '__root__', label: t('codex.slashCommands.folders.root'), icon: Home, count: stats.value.rootCount },
...
const loadCommands = async () => {
  try {
    ...
    alert(t('codex.slashCommands.messages.loadFailed'))
...
const handleSubmit = async () => {
  if (!formData.value.name || !formData.value.command) { alert(t('codex.slashCommands.validation.required')); return }
  ...
  try {
    ...
    editingCommand.value = null
    loadCommands()
  } catch (err) { console.error('操作失败:', err); alert(t('codex.slashCommands.messages.operationFailed')) }
}

const handleDelete = async (name: string) => {
  if (!confirm(t('codex.slashCommands.deleteConfirm', { name }))) return
  try { await deleteCodexSlashCommand(name); loadCommands() } catch (err) { console.error('删除失败:', err); alert(t('codex.slashCommands.messages.deleteFailed')) }
}

const handleToggle = async (name: string) => {
  try { await toggleCodexSlashCommand(name); loadCommands() } catch (err) { console.error('切换状态失败:', err); alert(t('codex.slashCommands.messages.toggleFailed')) }
}
