  ...
  <div class="stats-view p-6 space-y-6 min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-950">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      ...
      <div class="flex items-center gap-3">
        ...
        <select
          ...
          @change="loadData"
        >
          <option value="today">{{ $t('stats.timeRange.today') }}</option>
          <option value="week">{{ $t('stats.timeRange.thisWeek') }}</option>
          <option value="month">{{ $t('stats.timeRange.thisMonth') }}</option>
        </select>
    ...
    <div v-if="loading" class="flex items-center justify-center py-16">
      <div class="flex flex-col items-center gap-4">
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400">{{ $t('stats.states.loading', '加载中...') }}</p>
      </div>
    </div>

    <!-- Error State -->
    <div v-if="error" class="rounded-2xl bg-red-50/80 dark:bg-red-900/20 border border-red-200 dark:border-red-800 p-6 backdrop-blur-sm">
      <div class="flex items-start gap-4">
        ...
        <div>
          <h3 class="text-sm font-semibold text-red-800 dark:text-red-200">{{ $t('stats.states.loadFailed') }}</h3>
    ...
    <div v-if="!loading && !error && stats" class="space-y-6">
      ...
      <div class="p-6 rounded-2xl bg-white/80 dark:bg-gray-800/80 border border-gray-200/50 dark:border-gray-700/50 shadow-sm backdrop-blur-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div class="p-4 rounded-xl bg-gray-50/50 dark:bg-gray-700/30">
            <p class="text-sm text-gray-600 dark:text-gray-400">{{ $t('stats.tokenDetails.cacheToken') }}</p>
            <p class="text-xl font-bold text-gray-900 dark:text-white mt-1">{{ formatNumber(stats.token_stats.total_cache_tokens) }}</p>
          </div>
          <div class="p-4 rounded-xl bg-gray-50/50 dark:bg-gray-700/30">
            <p class="text-sm text-gray-600 dark:text-gray-400">{{ $t('stats.tokenDetails.cacheEfficiency') }}</p>
            <p class="text-xl font-bold text-gray-900 dark:text-white mt-1">{{ formatPercent(stats.token_stats.cache_efficiency) }}%</p>
          </div>
          <div class="p-4 rounded-xl bg-gray-50/50 dark:bg-gray-700/30">
            <p class="text-sm text-gray-600 dark:text-gray-400">{{ $t('stats.tokenDetails.totalToken') }}</p>
            <p class="text-xl font-bold text-gray-900 dark:text-white mt-1">{{ formatNumber(getTotalTokens()) }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="p-6 rounded-2xl bg-white/80 dark:bg-gray-800/80 border border-gray-200/50 dark:border-gray-700/50 shadow-sm backdrop-blur-sm">
          ...
          <div class="space-y-2 max-h-64 overflow-y-auto scrollbar-thin">
            ...
            <div v-if="Object.keys(stats.by_model).length === 0" class="text-center text-gray-500 dark:text-gray-400 py-8">
        ...
        <div class="p-6 rounded-2xl bg-white/80 dark:bg-gray-800/80 border border-gray-200/50 dark:border-gray-700/50 shadow-sm backdrop-blur-sm">
          ...
          <div class="space-y-2 max-h-64 overflow-y-auto scrollbar-thin">
            ...
            <div v-if="Object.keys(stats.by_project).length === 0" class="text-center text-gray-500 dark:text-gray-400 py-8">
              {{ $t('stats.states.noData') }}
            </div>
          </div>
        </div>
      ...
      <!-- Cost Trend -->
      <div v-if="stats.trend && stats.trend.length > 0" class="p-6 rounded-2xl bg-white/80 dark:bg-gray-800/80 border border-gray-200/50 dark:border-gray-700/50 shadow-sm backdrop-blur-sm">
        <h2 class="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
          <svg class="w-5 h-5 text-orange-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
          ...
          >
            <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ daily.date }}</p>
            <p class="text-lg font-bold text-gray-900 dark:text-white mt-1">${{ formatCost(daily.cost) }}</p>
            <p class="text-xs text-gray-500 dark:text-gray-400">{{ daily.count }} {{ $t('stats.units.times') }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- Empty State -->
    <div v-if="!loading && !error && stats && stats.record_count === 0" class="p-16 rounded-2xl bg-white/80 dark:bg-gray-800/80 border border-gray-200/50 dark:border-gray-700/50 shadow-sm backdrop-blur-sm text-center">
      ...
      <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ $t('stats.states.noStatsData') }}</h3>
      <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">{{ $t('stats.states.noStatsHint') }}</p>
    </div>
      ...
      >
        <div class="w-full max-w-xl bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-2xl m-4 border border-gray-200/50 dark:border-gray-700/50">
          <div class="flex items-center justify-between mb-6">
            <div>
              <h3 class="text-xl font-bold text-gray-900 dark:text-white">{{ $t('stats.sections.providerUsage') }}</h3>
              <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">{{ $t('stats.sections.providerUsageSubtitle') }}</p>
            </div>
            <button
          ...
          <div class="space-y-4 max-h-[60vh] overflow-y-auto">
            ...
            <div v-if="sortedProviders.length === 0" class="text-center text-gray-500 dark:text-gray-400 py-8">
              {{ $t('stats.states.noData') }}
            </div>
          </div>
        </div>
