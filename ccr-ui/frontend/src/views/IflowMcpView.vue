    ...
    <div class="max-w-[1800px] mx-auto">
      ...
      <Breadcrumb
        :items="[
          ...
          { label: $t('iflow.mcp.title'), path: '/iflow/mcp', icon: Server }
        ]"
        module-color="#f97316"
      />
          ...
          <div class="flex items-center justify-between mb-6">
            ...
            <div class="flex items-center gap-3">
              ...
              <button
                ...
                @click="handleAdd"
              >
                <Plus class="w-4 h-4" />{{ $t('iflow.mcp.addServer') }}
              </button>
            </div>
          </div>
                ...
                <div class="flex-1">
                  ...
                  <div class="space-y-2 text-sm">
                    <div v-if="server.command">
                      <span :style="{ color: 'var(--text-muted)' }">{{ $t('iflow.mcp.command') }}:</span>
                      <code
                    ...
                    <div v-if="server.url">
                      <span :style="{ color: 'var(--text-muted)' }">{{ $t('iflow.mcp.url') }}:</span>
                      <code
                        class="ml-2 px-2 py-1 rounded font-mono"
                        :style="{ background: 'var(--bg-secondary)', color: 'var(--accent-primary)' }"
                      >{{ server.url }}</code>
                    ...
                    <div v-if="server.args && server.args.length > 0">
                      <span :style="{ color: 'var(--text-muted)' }">{{ $t('iflow.mcp.args') }}:</span>
                      <code
                        class="ml-2 px-2 py-1 rounded font-mono"
                    ...
                    <div v-if="server.env && Object.keys(server.env).length > 0">
                      <span :style="{ color: 'var(--text-muted)' }">{{ $t('iflow.mcp.env') }}:</span>
          ...
          >
            ...
            >
              <h2
                ...
                :style="{ color: 'var(--text-primary)' }"
              >
                {{ editingServer ? $t('iflow.mcp.editServer') : $t('iflow.mcp.addServer') }}
              </h2>
              ...
              <div class="mb-4">
                <label class="flex items-center gap-2 cursor-pointer">
                  ...
                  >{{ $t('iflow.mcp.httpServerHint') }}</span>
                </label>
              </div>
              ...
              <div class="space-y-4">
                <div v-if="isHttpServer">
                  <label
                    ...
                    :style="{ color: 'var(--text-secondary)' }"
                  >{{ $t('iflow.mcp.serverUrl') }} *</label>
                  <input
                    v-model="formData.url"
                    type="text"
                    class="w-full px-3 py-2 rounded-lg"
                    :style="{ background: 'var(--bg-tertiary)', border: '1px solid var(--border-color)', color: 'var(--text-primary)' }"
                    :placeholder="$t('iflow.mcp.urlPlaceholder')"
                  >
                </div>
                ...
                <div v-else>
                  <label
                    class="block text-sm font-semibold mb-1"
                    :style="{ color: 'var(--text-secondary)' }"
                  >{{ $t('iflow.mcp.command') }} *</label>
                  <input
                    v-model="formData.command"
                    type="text"
                    class="w-full px-3 py-2 rounded-lg font-mono"
                    :style="{ background: 'var(--bg-tertiary)', border: '1px solid var(--border-color)', color: 'var(--text-primary)' }"
                    :placeholder="$t('iflow.mcp.commandPlaceholder')"
                  >
                </div>
                ...
                <div v-if="!isHttpServer">
                  <label
                    class="block text-sm font-semibold mb-1"
                    :style="{ color: 'var(--text-secondary)' }"
                  >{{ $t('iflow.mcp.args') }}</label>
                  <input
                    v-model="argInput"
                    type="text"
                    class="w-full px-3 py-2 rounded-lg font-mono"
                    :style="{ background: 'var(--bg-tertiary)', border: '1px solid var(--border-color)', color: 'var(--text-primary)' }"
                    :placeholder="$t('iflow.mcp.argsPlaceholder')"
                  >
                  <div
                    class="text-xs mt-1"
                    :style="{ color: 'var(--text-muted)' }"
                  >
                    {{ $t('iflow.mcp.argsHint') }}
                  </div>
                </div>
...
import { Server, Plus, Edit2, Trash2, ArrowLeft, Home, Flame } from 'lucide-vue-next'
...
const handleSubmit = async () => {
  if (!isHttpServer.value && !formData.value.command) { alert(t('iflow.mcp.validation.commandRequired')); return }
  if (isHttpServer.value && !formData.value.url) { alert(t('iflow.mcp.validation.urlRequired')); return }
  ...
  try {
    ...
    if (editingServer.value) {
      ...
      alert(t('iflow.mcp.updateSuccess'))
    } else {
      await addIflowMcpServer(request)
      alert(t('iflow.mcp.addSuccess'))
    }
    showAddForm.value = false
    await loadServers()
  } catch (err) { alert(t('iflow.mcp.operationFailed', { error: err instanceof Error ? err.message : 'Unknown error' })) }
}

const handleDelete = async (name: string) => {
  if (!confirm(t('iflow.mcp.confirmDelete', { name }))) return
  try {
    await deleteIflowMcpServer(name)
    alert(t('iflow.mcp.deleteSuccess'))
    await loadServers()
  } catch (err) { alert(t('iflow.mcp.deleteFailed', { error: err instanceof Error ? err.message : 'Unknown error' })) }
}
