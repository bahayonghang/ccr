        ...
        <div class="max-w-[1600px] mx-auto">
          <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-4">
              ...
              >
                <Bot class="inline-block w-7 h-7 mr-2" />{{ $t('iflow.agents.title') }}
              </h2>
              <span
            ...
            <div class="flex items-center gap-3">
              ...
              <button
                ...
                @click="handleAdd"
              >
                <Plus class="inline-block w-5 h-5 mr-2" />{{ $t('iflow.agents.addAgent') }}
              </button>
            </div>
          </div>
          ...
          <div class="mb-4">
            <div class="relative">
              ...
              <input
                v-model="searchQuery"
                type="text"
                :placeholder="$t('iflow.agents.searchPlaceholder')"
            ...
            <p
              ...
              class="mt-2 text-sm"
              :style="{ color: 'var(--text-muted)' }"
              v-html="$t('iflow.agents.searchResults', { count: filteredAgents.length })"
            />
          </div>
          ...
          <div class="space-y-4">
            ...
            <div
              ...
              :style="{ color: 'var(--text-muted)' }"
            >
              <Search class="w-12 h-12 mx-auto mb-3 opacity-50" /><p>{{ $t('iflow.agents.noResults') }}</p><p class="text-sm mt-2">
                {{ $t('iflow.agents.tryDifferentKeywords') }}
              </p>
            </div>
            <div
              ...
              <div class="flex items-start justify-between">
                ...
                <div class="flex gap-2 ml-4">
                  <button
                    ...
                    :title="agent.disabled ? $t('common.enable') : $t('common.disable')"
    ...
    >
      ...
      >
        <h3
          ...
          :style="{ color: 'var(--text-primary)' }"
        >
          {{ editingAgent ? $t('iflow.agents.editAgent') : $t('iflow.agents.addAgent') }}
        </h3>
        <div class="space-y-4">
          <div>
            <label
          ...
          <div>
            <label
              class="block mb-2 font-medium"
              :style="{ color: 'var(--text-secondary)' }"
            >{{ $t('iflow.agents.tools') }}</label>
            <div class="flex gap-2 mb-2">
              <input
                v-model="toolInput"
                type="text"
                :placeholder="$t('iflow.agents.toolPlaceholder')"
          ...
          <div>
            <label
              class="block mb-2 font-medium"
              :style="{ color: 'var(--text-secondary)' }"
            >{{ $t('iflow.agents.systemPrompt') }}</label>
            <textarea
...
const folderOptions = computed(() => [
  { value: '', label: t('iflow.agents.folders.all'), icon: Folder, count: stats.value.total },
  { value: '__root__', label: t('iflow.agents.folders.root'), icon: Home, count: stats.value.rootCount },
...
const loadAgents = async () => {
  try {
    ...
    alert(t('iflow.agents.loadFailed'))
...
const handleSubmit = async () => {
  if (!formData.value.name || !formData.value.model) { alert(t('iflow.agents.validation.requiredFields')); return }
  ...
  try {
    ...
    editingAgent.value = null
    loadAgents()
  } catch (err) { console.error('操作失败:', err); alert(t('iflow.agents.operationFailed')) }
}

const handleDelete = async (name: string) => {
  if (!confirm(t('iflow.agents.confirmDelete', { name }))) return
  try { await deleteIflowAgent(name); loadAgents() } catch (err) { console.error('删除失败:', err); alert(t('iflow.agents.deleteFailed')) }
}

const handleToggle = async (name: string) => {
  try { await toggleIflowAgent(name); loadAgents() } catch (err) { console.error('切换状态失败:', err); alert(t('iflow.agents.toggleFailed')) }
}
