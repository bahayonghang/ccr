    ...
    <div style="display: flex; gap: 0">
      ...
      <div :style="{ width: '180px', background: 'var(--bg-secondary)', borderRight: '1px solid var(--border-color)', padding: '12px 8px', overflowY: 'auto', height: 'calc(100vh - 40px)', flexShrink: 0 }">
        <h4 :style="{ color: 'var(--text-muted)', fontSize: '11px', fontWeight: '600', marginBottom: '8px', marginLeft: '8px', textTransform: 'uppercase', letterSpacing: '0.5px' }">
          {{ $t('gemini.slashCommands.folders.label') }}
        </h4>
        <div
        ...
        <div class="max-w-[1600px] mx-auto">
          <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-4">
              ...
              >
                <Command class="inline-block w-7 h-7 mr-2" />{{ $t('gemini.slashCommands.pageTitle') }}
              </h2>
              <span
            ...
            <div class="flex items-center gap-3">
              ...
              >
                <Home class="w-4 h-4" /><span>{{ $t('gemini.slashCommands.backToHome') }}</span>
              </RouterLink>
              <button
                ...
                @click="handleAdd"
              >
                <Plus class="inline-block w-5 h-5 mr-2" />{{ $t('gemini.slashCommands.addCommandShort') }}
              </button>
            </div>
          </div>
          ...
          <div class="mb-4">
            <div class="relative">
              ...
              <input
                v-model="searchQuery"
                type="text"
                :placeholder="$t('gemini.slashCommands.searchPlaceholder')"
            ...
            <p
              ...
              class="mt-2 text-sm"
              :style="{ color: 'var(--text-muted)' }"
              v-html="$t('gemini.slashCommands.searchResults', { count: filteredCommands.length })"
            />
          </div>
          ...
          <div class="space-y-4">
            <div
              ...
              :style="{ color: 'var(--text-muted)' }"
            >
              {{ $t('gemini.slashCommands.loading') }}
            </div>
            <div
              ...
              :style="{ color: 'var(--text-muted)' }"
            >
              {{ $t('gemini.slashCommands.emptyState') }}
            </div>
            <div
              ...
              :style="{ color: 'var(--text-muted)' }"
            >
              <Search class="w-12 h-12 mx-auto mb-3 opacity-50" /><p>{{ $t('gemini.slashCommands.noResults') }}</p><p class="text-sm mt-2">
                {{ $t('gemini.slashCommands.noResultsHint') }}
              </p>
            </div>
            <div
              ...
              <div class="flex items-start justify-between">
                <div class="flex-1">
                  <div class="flex items-center gap-3 mb-2">
                    ...
                    >{{ $t('gemini.slashCommands.disabledBadge') }}</span>
                  </div>
                  <p
                    class="mb-2"
                ...
                <div class="flex gap-2 ml-4">
                  <button
                    ...
                    :title="cmd.disabled ? $t('gemini.slashCommands.enable') : $t('gemini.slashCommands.disable')"
    ...
    >
      ...
      >
        <h3
          ...
          :style="{ color: 'var(--text-primary)' }"
        >
          {{ editingCommand ? $t('gemini.slashCommands.editCommandShort') : $t('gemini.slashCommands.addCommandShort') }}
        </h3>
        <div class="space-y-4">
          <div>
            <label
            ...
            >{{ $t('gemini.slashCommands.nameLabel') }}</label>
            <input
          ...
          <div>
            <label
              class="block mb-2 font-medium"
              :style="{ color: 'var(--text-secondary)' }"
            >{{ $t('gemini.slashCommands.commandLabel') }}</label>
            <input
          ...
          <div>
            <label
              class="block mb-2 font-medium"
              :style="{ color: 'var(--text-secondary)' }"
            >{{ $t('gemini.slashCommands.descriptionLabel') }}</label>
            <textarea
        ...
        <div class="flex gap-3 mt-6">
          <button
            ...
            @click="handleSubmit"
          >
            {{ editingCommand ? $t('gemini.slashCommands.save') : $t('gemini.slashCommands.add') }}
          </button>
          <button
            class="flex-1 px-6 py-3 rounded-lg font-medium transition-all hover:scale-105"
...
const folderOptions = computed(() => [
  { value: '', label: t('gemini.slashCommands.folders.all'), icon: Folder, count: stats.value.total },
  { value: '__root__', label: t('gemini.slashCommands.folders.root'), icon: Home, count: stats.value.rootCount },
...
const loadCommands = async () => {
  try {
    ...
    alert(t('gemini.slashCommands.messages.loadFailed'))
...
const handleSubmit = async () => {
  if (!formData.value.name || !formData.value.command) { alert(t('gemini.slashCommands.validation.required')); return }
  ...
  try {
    ...
    editingCommand.value = null
    loadCommands()
  } catch (err) { console.error('操作失败:', err); alert(t('gemini.slashCommands.messages.operationFailed', { error: err instanceof Error ? err.message : 'Unknown error' })) }
}

const handleDelete = async (name: string) => {
  if (!confirm(t('gemini.slashCommands.deleteConfirm', { name }))) return
  try { await deleteGeminiSlashCommand(name); loadCommands() } catch (err) { console.error('删除失败:', err); alert(t('gemini.slashCommands.messages.deleteFailed', { error: err instanceof Error ? err.message : 'Unknown error' })) }
}

const handleToggle = async (name: string) => {
  try { await toggleGeminiSlashCommand(name); loadCommands() } catch (err) { console.error('切换状态失败:', err); alert(t('gemini.slashCommands.messages.toggleFailed', { error: err instanceof Error ? err.message : 'Unknown error' })) }
}
