...
## 🛠️ 技能 (Skills) 管理 (v3.8+)
跨平台统一技能管理中心，扩展 AI CLI 能力。

> 📖 **详细文档**：[技能管理详细指南](./skills.md)

**支持平台**：Claude Code、Codex、Gemini CLI、Qwen、iFlow、Droid

### 核心功能

#### 📦 统一技能中心 (SkillHub)
- **已安装管理**：统一查看、编辑、删除所有平台的已安装技能
- **多维筛选**：按平台、来源、分类、标签过滤技能
- **技能详情**：查看 SKILL.md 内容、元数据、编辑技能
- **操作日志**：记录所有技能安装/卸载操作

#### 🏪 技能市场 (skills.sh)
- **热门浏览**：浏览 skills.sh 市场热门技能
- **关键词搜索**：实时搜索市场技能
- **一键安装**：选择目标平台后直接安装
- **批量安装**：批量选择多个技能一次安装
- **缓存机制**：市场数据缓存，支持手动刷新
...
#### ➕ 多源安装
- **市场安装**：从 skills.sh 一键安装
- **GitHub 导入**：输入仓库 URL 直接导入技能
- **本地导入**：从本地文件夹导入自定义技能
- **npx 安装**：通过 npm 包安装技能（自动检测 npx 可用性）

#### 🎯 多平台目标
- 自动检测已安装的 AI CLI 平台
- 安装时可选择一个或多个目标平台
- 快捷「选择已检测」一键勾选所有已安装平台

### 常见场景

| 场景 | 操作路径 |
|------|----------|
| 快速安装热门技能 | 添加技能 → 市场浏览 → 安装 |
| 批量装备新平台 | 添加技能 → 批量模式 → 多选 → 批量安装 |
| 导入自定义技能 | 添加技能 → 手动安装 → GitHub/本地/npx |
| 管理已有技能 | 技能主页 → 筛选 → 查看/编辑/删除 |
