...
- ✅ **多维度统计** - 按时间、模型、项目分组分析
- ✅ **提供商聚类** - 按 provider/platform 聚合调用次数，快速看到 Claude/Codex/Gemini 等占比
- ✅ **可视化仪表板** - 直观的数据展示
- ✅ **趋势分析** - 查看每日成本趋势
- ✅ **数据导出** - 支持 JSON 格式导出
- ✅ **响应式设计** - 支持桌面和移动端
...
- **入口**: 页头右侧 “提供商统计” 按钮，点击弹出模态框
- **内容**: provider → 调用次数 的列表 + 水平柱状图，按次数降序展示
- **空数据**: 显示“暂无数据”而不是空白区域
- **数据源**: `/api/stats/provider-usage` 汇总 `~/.ccr/platforms/claude/profiles.toml` 中各配置的 `usage_count`
- **用途**: 快速看出各平台（Claude/Codex/Gemini/Qwen/unknown）调用量占比，辅助路由与配额决策
